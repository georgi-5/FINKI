...
class BTree<E> {
    ...
    public int countChild(BNode<E> node,int counter){
        if(node==null){
            return counter;
        }
        if(node.left!=null && node.right!=null){
        ...
        counter=countChild(node.left,counter);
        counter=countChild(node.right,counter);
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i=0; i<n+q; i++){
            ...
            if(command.equals("root")){
                ...
                int counter=0;
                BNode<String> newNode=tree.find(root,parrent);
                System.out.println(tree.countChild(newNode,counter));
            }
        }
