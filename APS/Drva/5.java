...
interface Stack<E> {
 ...
 class BTree<E> {
    ...
    public BNode<String > getNode(BNode<String> root,String node) {
        if(root == null){
            return null;
        }
        if(root.info.equals(node)){
            return root;
        }
        BNode<String> leftNode = getNode(root.left,node);
        if(leftNode != null) {
            return leftNode;
        }
        BNode<String> rightNode = getNode(root.right,node);
        if(rightNode != null) {
            return rightNode;
        }
        return null;
    ...
    public static void main(String[] args){
       ...
       for(int i=0;i<n+q;i++) {
           ...
           if (tokens[0].equals("root")) {
               tree.makeRoot(tokens[1]);
           } else {

               if (tokens[0].equals("add")) {
                   //add bravo echo LEFT
                   //0    1       2     3
                   BNode<String> newNode = tree.getNode(root, tokens[1]);
                   if (tokens[3].equals("LEFT")) {
                       tree.addChild(newNode, BNode.LEFT, tokens[2]);

                   } else if (tokens[3].equals("RIGHT")) {
                       tree.addChild(newNode, BNode.RIGHT, tokens[2]);

                   }
               } else if (tokens[0].equals("ask")) {
                   BNode<String> newNode = tree.getNode(root, tokens[1]);
                   System.out.println(tree.insideNodesR(newNode)-1);
               }
