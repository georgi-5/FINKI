...
class prog{
    public static void main(String[] args) {
        ...
        CBHT<String,List<Person>> map=new CBHT<>(n*n);
        for(int i=0;i<n;i++){
            ...
            String []timez=time.split(":");
            String first=timez[0];
            String last=timez[1];
            String finalT=first+last;
            if(salary>=spent) {
                ...
                if (map.search(city) == null) {
                    ...
                    persons.add(person);
                    map.insert(city, persons);
                } else {
                    List<Person> list=map.search(city).element.value;
                    list.add(person);
                    map.insert(city, list);
                }
            }
        }
        sc.nextLine();
        ...
        for(int i=0;i<list.length;i++){
            System.out.println("City: "+list[i]+" has the following number of customers:");
            System.out.println(map.search(list[i]).element.value.size());
            System.out.println("The user who spent the most purchasing for that city is:");
            Person max=new Person();
            for(int j=0;j<map.search(list[i]).element.value.size();j++){
                if(map.search(list[i]).element.value.get(j).getSpent()>max.getSpent()){
                    max=map.search(list[i]).element.value.get(j);
                }
            }
            System.out.println(max.getName()+" "+max.getSurname()+" with salary "+max.getSalary()+" from address "+max.getIp()+" who spent "+max.getSpent());
            System.out.println();
        }
