...
//        Map<String,List<Person>> map=new HashMap<>();
        OBHT<String,List<Person>> map=new OBHT<>(n*n);
        for(int i=0;i<n;i++){
            ...
            if(timez>=1159) {
                ...
                if(map.search(city)==-1){
                    List<Person> list=new ArrayList<>();
                    list.add(person);
                    map.insert(city,list);
                }
                else{
                    List<Person> list=map.getBucket(map.search(city)).value;
                    list.add(person);
                    map.insert(city,list);
                }
            }
        ...
        for(int i=0;i<list.length;i++){
            ...
            System.out.println(map.getBucket(map.search(list[i])).value.size());
            System.out.println("The user who logged on earliest after noon from that city is:");
            Person max=new Person();
            for(int j=0;j<map.getBucket(map.search(list[i])).value.size();j++){
                if(map.getBucket(map.search(list[i])).value.get(j).getMax()<max.getMax()){
                    max=map.getBucket(map.search(list[i])).value.get(j);
                }
            }
